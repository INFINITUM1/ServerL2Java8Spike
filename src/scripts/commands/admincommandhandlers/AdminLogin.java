...
public class AdminLogin implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		{
			gmOnly();
			activeChar.sendAdmResultMessage("Server is now GM only");
			showMainPage(activeChar);
		}
		else if(command.equals("admin_server_all"))
		{
			allowToAll();
			activeChar.sendAdmResultMessage("Server is not GM only anymore");
			showMainPage(activeChar);
		}
		else if(command.startsWith("admin_server_max_player"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				try
				{
					...
					activeChar.sendAdmResultMessage("maxPlayer set to "+new Integer(number).intValue());
					showMainPage(activeChar);
				}
				catch(NumberFormatException e)
				{
					activeChar.sendAdmResultMessage("Max players must be a number.");
				}
			}
			else
			{
				activeChar.sendAdmResultMessage("Format is server_max_player <max>");
			}
		}
		else if(command.startsWith("admin_server_list_clock"))
		{
			...
			if (st.countTokens() > 1)
			{
				...
				if(mode.equals("on"))
				{
					...
					activeChar.sendAdmResultMessage("A clock will now be displayed next to the server name");
				...
				else if(mode.equals("off"))
				{
					...
					activeChar.sendAdmResultMessage("The clock will not be displayed");
					showMainPage(activeChar);
				}
				else
				{
					activeChar.sendAdmResultMessage("Format is server_list_clock <on/off>");
				}
			}
			else
			{
