...
public class AdminPathNode implements IAdminCommandHandler
{
    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) 
	{
    	...
    	else if(command.equals("admin_find_path"))
        {
    		if (Config.GEODATA < 2)
    		{
    			activeChar.sendAdmResultMessage("PathFinding has not been enabled.");
    			return true;
    		}
    		if (activeChar.getTarget() != null)
            {
                ...
                if (path == null)
                {
                	activeChar.sendAdmResultMessage("No Route!");
                	return true;
                }
    			for(AbstractNodeLoc a : path)
                {
                	activeChar.sendAdmResultMessage("x:"+a.getX()+" y:"+a.getY()+" z:"+a.getZ());
                }
            }
            else
                activeChar.sendAdmResultMessage("No Target!");
        }*/
        return true;
    }
