	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		if (command.startsWith("admin_cw_info"))
		{
			if (!command.contains("menu"))
			{
				...
				{
					activeChar.sendAdmResultMessage("> "+cw.getName()+" ("+cw.getItemId()+")");
					if (cw.isActivated())
					{
						L2PcInstance pl = cw.getPlayer();
						activeChar.sendAdmResultMessage("  Player holding: "+ pl==null ? "null" : pl.getName());
						activeChar.sendAdmResultMessage("    Player karma: "+cw.getPlayerKarma());
						activeChar.sendAdmResultMessage("    Time Remaining: "+(cw.getTimeLeft()/60000)+" min.");
						activeChar.sendAdmResultMessage("    Kills : "+cw.getNbKills());
					}
