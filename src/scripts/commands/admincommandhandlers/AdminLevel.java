    ...
    public boolean useAdminCommand(String command, L2PcInstance activeChar) {
        ...
        if (actualCommand.equalsIgnoreCase("admin_add_level")) {
            try {
                ...
                activeChar.sendAdmResultMessage("Wrong Number Format");
            }
        } else if (actualCommand.equalsIgnoreCase("admin_set_level")) {
            try {
                if (targetChar == null || !(targetChar.isPlayer())) {
                    activeChar.sendPacket(SystemMessage.id(SystemMessageId.TARGET_IS_INCORRECT));	// incorrect target!
                    return false;
                }
                ...
                if (lvl >= 1 && lvl <= Experience.MAX_LEVEL) {
                    ...
                    activeChar.sendAdmResultMessage("You must specify level between 1 and " + Experience.MAX_LEVEL + ".");
                    return false;
                }
            } catch (NumberFormatException e) {
                activeChar.sendAdmResultMessage("You must specify level between 1 and " + Experience.MAX_LEVEL + ".");
