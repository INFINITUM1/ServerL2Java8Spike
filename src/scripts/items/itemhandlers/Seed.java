...
public class Seed implements IItemHandler {
    ...
    public void useItem(L2PlayableInstance playable, L2ItemInstance item, boolean ctrl) {
        ...
        if(target == null || !(target.isL2Npc())) 
        {
            _activeChar.sendPacket(Static.INCORRECT_TARGET);
        ...
        if ( !(target.isL2Monster())  ||
        	...
        	_activeChar.sendPacket(Static.THE_TARGET_IS_UNAVAILABLE_FOR_SEEDING);
        	_activeChar.sendActionFailed();
        	return;
        }
        ...
        if(areaValid(MapRegionTable.getInstance().getAreaCastle(_activeChar))) {
        	...
        	_activeChar.sendPacket(Static.THIS_SEED_MAY_NOT_BE_SOWN_HERE);
        }
    }
