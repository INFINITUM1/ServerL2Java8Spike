...
public class L2Fishing implements Runnable {
    ...
    public void run() {
        ...
        if (_fishCurHp >= _fishMaxHp * 2) {
            // The fish got away
            _fisher.sendPacket(SystemMessage.id(SystemMessageId.BAIT_STOLEN_BY_FISH));
            doDie(false);
        } else if (_time <= 0) {
            // Time is up, so that fish got away
            _fisher.sendPacket(SystemMessage.id(SystemMessageId.FISH_SPIT_THE_HOOK));
            doDie(false);
        } else {
            aiTask();
        }
    ...
    public L2Fishing(L2PcInstance Fisher, FishData fish, boolean isNoob, boolean isUpperGrade) {
        ...
        _fisher.sendPacket(SystemMessage.id(SystemMessageId.GOT_A_BITE));

        if (_fishAiTask == null) {
            _fishAiTask = EffectTaskManager.getInstance().scheduleAtFixedRate(this, 1000, 1000);
        }
    ...
    public synchronized void doDie(boolean win) {
        ...
        if (win) {
            ...
            if (check <= 5) {
                ...
                _fisher.sendPacket(SystemMessage.id(SystemMessageId.YOU_CAUGHT_SOMETHING));
    ...
    public void useRealing(int dmg, int pen) {
        _anim = 2;
        if (Rnd.get(100) > 90) {
            _fisher.sendPacket(SystemMessage.id(SystemMessageId.FISH_RESISTED_ATTEMPT_TO_BRING_IT_IN));
            _goodUse = 0;
            changeHp(0, pen);
            return;
        }
        ...
        if (_mode == 1) {
            if (_deceptiveMode == 0) {
                ...
                SystemMessage sm = SystemMessage.id(SystemMessageId.REELING_SUCCESFUL_S1_DAMAGE);
                sm.addNumber(dmg);
                _fisher.sendPacket(sm);
                if (pen == 50) {
                    sm = SystemMessage.id(SystemMessageId.REELING_SUCCESSFUL_PENALTY_S1);
                    sm.addNumber(pen);
                    _fisher.sendPacket(sm);
                }
                _goodUse = 1;
                ...
                SystemMessage sm = SystemMessage.id(SystemMessageId.FISH_RESISTED_REELING_S1_HP_REGAINED);
                sm.addNumber(dmg);
                _fisher.sendPacket(sm);
                _goodUse = 2;
                changeHp(-dmg, pen);
    ...
    public void usePomping(int dmg, int pen) {
        ...
        if (_mode == 0) {
            if (_deceptiveMode == 0) {
                ...
                SystemMessage sm = SystemMessage.id(SystemMessageId.PUMPING_SUCCESFUL_S1_DAMAGE);
                sm.addNumber(dmg);
                _fisher.sendPacket(sm);
                if (pen == 50) {
                    sm = SystemMessage.id(SystemMessageId.PUMPING_SUCCESSFUL_PENALTY_S1);
                    sm.addNumber(pen);
                    _fisher.sendPacket(sm);
                }
                _goodUse = 1;
                ...
                SystemMessage sm = SystemMessage.id(SystemMessageId.FISH_RESISTED_PUMPING_S1_HP_REGAINED);
                sm.addNumber(dmg);
                _fisher.sendPacket(sm);
                _goodUse = 2;
                changeHp(-dmg, pen);
    ...
    private void PenaltyMonster() {
        ...
        _fisher.sendPacket(SystemMessage.id(SystemMessageId.YOU_CAUGHT_SOMETHING_SMELLY_THROW_IT_BACK));
        switch (lvl) {
            case 0:
            case 1:
                npcid = 18319;
