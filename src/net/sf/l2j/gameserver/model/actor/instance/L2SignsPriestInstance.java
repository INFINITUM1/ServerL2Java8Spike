        ...
        else if (command.startsWith("SevenSigns"))
        {
            ...
            switch (val)
            {
                case 2: // Purchase Record of the Seven Signs
                    ...
                    sendPacket(su);
                    player.sendPacket(SystemMessage.id(SystemMessageId.EARNED_ITEM).addItemName(SevenSigns.RECORD_SEVEN_SIGNS_ID));
                    break;
...
*/
                            else
                            {
                                ...
                                if (cabal == SevenSigns.CABAL_DAWN)
                                {
                                    ...
                                    {
                                        allowJoinDawn = true;
                                        player.sendPacket(SystemMessage.id(SystemMessageId.DISSAPEARED_ITEM).addNumber(1).addItemName(SevenSigns.CERTIFICATE_OF_APPROVAL_ID));
                                    }
                                    else if (player.reduceAdena("SevenSigns", SevenSigns.ADENA_JOIN_DAWN_COST, this, false))
                                    {
                                        allowJoinDawn = true;
                                        player.sendPacket(SystemMessage.id(SystemMessageId.DISSAPEARED_ADENA).addNumber(SevenSigns.ADENA_JOIN_DAWN_COST));
                                    }
                    ...
                    if (cabal == SevenSigns.CABAL_DAWN) player.sendPacket(Static.SEVENSIGNS_PARTECIPATION_DAWN); // Joined Dawn
                    else player.sendPacket(Static.SEVENSIGNS_PARTECIPATION_DUSK); // Joined Dusk
    ...
    private final boolean getPlayerAllyHasCastle(L2PcInstance player)
    {
        ...
        if (!Config.ALT_GAME_REQUIRE_CLAN_CASTLE)
        {
            ...
            if (allyId != 0)
            {
                ...
                for (L2Clan clan : cn)
                    if (clan.getAllyId() == allyId) if (clan.getHasCastle() > 0) 
						return true;
            }
        }
