...
public class L2SummonInstance extends L2Summon {
    ...
    if (crit)
    {
    getOwner().sendPacket(SystemMessage.id(SystemMessage.SUMMON_CRITICAL_HIT));
    }
    
    if (miss)
    {
    getOwner().sendPacket(SystemMessage.id(SystemMessage.MISSED_TARGET));
    }
    else
    {
    SystemMessage sm = SystemMessage.id(SystemMessage.SUMMON_GAVE_DAMAGE_OF_S1);
    sm.addNumber(damage);
    getOwner().sendPacket(sm);
    }
    }
