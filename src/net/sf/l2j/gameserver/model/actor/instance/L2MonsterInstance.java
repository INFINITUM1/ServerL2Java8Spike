...
public class L2MonsterInstance extends L2Attackable {
    ...
    public boolean doDie(L2Character killer) {
        ...
        if (Config.MOB_KILL_REPUTATION) {
            rewardClanReputation(killer.getPlayer(), Config.MOB_KILL_REPUTATION_TABLE.get(getNpcId()));
        }

        return true;
    }

    private void rewardClanReputation(L2PcInstance player, Integer bonus) {
        if (player == null
                || bonus == null) {
            return;
        }
        ...
        player.getClan().incWarPoints(bonus);
    }
