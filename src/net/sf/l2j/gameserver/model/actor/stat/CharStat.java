...
public class CharStat {
     ...
     * on the targeted L2Character.<BR> <BR>
     *
     ...
     * stored in the value property of an Env class instance.<BR> <BR>
     *
    ...
    public final double calcStat(Stats stat, double init, L2Character target, L2Skill skill) {
        ...
        if (env.value <= 0) {
            switch (stat) {
                case MAX_HP:
                    if (_activeChar.isPlayer()){
                        System.out.println("##3##" + env.value);
                    }
                    //env.value = (int) (_activeChar.getTemplate().baseHpMax * 10) / (81 - _activeChar.getLevel());
                    //break;
                case MAX_MP:
                    //env.value = (int) (_activeChar.getTemplate().baseMpMax * 10) / (81 - _activeChar.getLevel());
                    //break;
                case MAX_CP:
                    //env.value = (int) (_activeChar.getTemplate().baseCpMax * 10) / (81 - _activeChar.getLevel());
                    //break;
                case MAGIC_DEFENCE:
                case POWER_DEFENCE:
                case POWER_ATTACK:
            ...
            env.value = _activeChar.getTemplate().baseHpMax;
            } else if (stat == Stats.MAX_MP) {
            env.value = _activeChar.getTemplate().baseMpMax;
            } else if (stat == Stats.MAX_CP) {
            env.value = _activeChar.getTemplate().baseCpMax;
            } else if ((stat == Stats.)
            || (stat == Stats.) || (stat == Stats.) || (stat == Stats.) || (stat == Stats.)
            || (stat == Stats.) || (stat == Stats.) || (stat == Stats.) || (stat == Stats.)
            || (stat == Stats.) || (stat == Stats.) || (stat == Stats.) || (stat == Stats.)) {
            env.value = 1;
            }*/
     ...
     * function of abnormal effects in progress.<BR> <BR>
     *
     * <B><U> Example of use </U> :</B><BR> <BR> <li> Calculate Magic damage
     * </li> <BR> <BR>
     *
    ...
    public int getMAtk(L2Character target, L2Skill skill) {
        ...
        if (Config.L2JMOD_CHAMPION_ENABLE_BLUE && _activeChar.isChampion1()) {
            bonusAtk = Config.L2JMOD_CHAMPION_ATK_BLUE;
        }
    ...
    public int getMAtkSpd() {
        ...
        if (Config.L2JMOD_CHAMPION_ENABLE_BLUE && _activeChar.isChampion1()) {
            bonusSpdAtk = Config.L2JMOD_CHAMPION_SPD_ATK_BLUE;
        }
    ...
    public final int getMCriticalHit(L2Character target, L2Skill skill) {
        ...
        //int rate = caclMcritRate(calcStat(Stats.MCRITICAL_RATE, _activeChar.getTemplate().baseMCritRate, target, skill));
        //_activeChar.sendAdmResultMessage("###mCritRate##" + rate + "%#");
        //return rate;
    ...
    private int caclMcritRate(double mrate) {
        mrate /= 10;
        mrate = Math.max(mrate, 10);
        mrate += _activeChar.calcStat(Stats.MCRITICAL_BONUS, 1, null, null);
    ...
    public int getPAtkSpd() {
        ...
        if (Config.L2JMOD_CHAMPION_ENABLE_BLUE && _activeChar.isChampion1()) {
            bonusAtk = Config.L2JMOD_CHAMPION_SPD_ATK_BLUE;
        }
