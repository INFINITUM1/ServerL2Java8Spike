...
{
    private static Logger _log = AbstractLogger.getLogger(L2TradeList.class.getName());
	...
	public synchronized void tradeItems(L2PcInstance player,L2PcInstance reciever)
	{
		Inventory playersInv = player.getInventory();
		Inventory recieverInv = reciever.getInventory();
		L2ItemInstance playerItem,recieverItem,newitem;
    ...
    private void changePetItemObjectId(int oldObjectId, int newObjectId)
    {
        ...
        try
        {
            ...
            statement = con.prepareStatement("UPDATE pets SET item_obj_id = ? WHERE item_obj_id = ?");
...
//		        seller.addAdena(cost);
		        ...
		        {
		            msg = SystemMessage.id(SystemMessageId.S1_PURCHASED_S2).addString(buyer.getName()).addItemName(sellerItem.getItemId());
		            sysmsgs.add(msg);
		            msg = SystemMessage.id(SystemMessageId.S1_PURCHASED_S2).addString("You").addItemName(sellerItem.getItemId());
		            sysmsgs.add(msg);
		        }
		        else
		        {
		            msg = SystemMessage.id(SystemMessageId.S1_PURCHASED_S3_S2_S).addString(buyer.getName()).addItemName(sellerItem.getItemId()).addNumber(amount);
		            sysmsgs.add(msg);
		            msg = SystemMessage.id(SystemMessageId.S1_PURCHASED_S3_S2_S).addString("You").addItemName(sellerItem.getItemId()).addNumber(amount);
		            sysmsgs.add(msg);
		        }
