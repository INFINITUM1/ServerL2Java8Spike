...
public class L2ClanMember {
    ...
    public void setPlayerInstance(L2PcInstance player) {
        ...
        if (player != null) {
            if (_clan.getLevel() > 3 && player.isClanLeader()) {
                ...
                if (Config.ANNOUNCE_CASTLE_OWNER && _clan.getHasCastle() > 0) {
                    //Announcements.getInstance().announceToAll(Static.CASTLE_LORD_LOGIN.replaceAll("%a%", player.getName()));
                    Announcements.getInstance().announceToAll(Static.CASTLE_LORD_LOGIN.replaceAll("%a%", player.getName()).replaceAll("%b%", _clan.getCastleName()));
                }
            }
    ...
    public void updateClanNpcScore(int bonus) {
        if (_player == null) {
            return;
        }

        _player.updateClanNpcScore(bonus);
    }
}
