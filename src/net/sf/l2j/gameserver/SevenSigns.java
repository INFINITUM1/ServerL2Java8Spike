...
{
    protected static final Logger _log = AbstractLogger.getLogger(SevenSigns.class.getName());
	...
	public SevenSigns()
	{
		...
		synchronized (this)
		{
			...
			double countDown = ((milliToChange / 1000) - numSecs) / (double) 60;
    ...
    protected void restoreSevenSignsData()
	{
    	...
    	try
    	{
		    ...
		    st = con.prepareStatement("SELECT char_obj_id, cabal, seal, red_stones, green_stones, blue_stones, ancient_adena_amount, contribution_score FROM seven_signs");
		    rs = st.executeQuery();

		    while (rs.next())
		    {
		    	int charObjId = rs.getInt("char_obj_id");
		    ...
		    st = con.prepareStatement("SELECT * FROM seven_signs_status WHERE id=0");
	        rs = st.executeQuery();
	        while (rs.next())
	        {
	        ...
	        st = con.prepareStatement("UPDATE seven_signs_status SET date=? WHERE id=0");
	        st.setInt(1, Calendar.getInstance().get(Calendar.DAY_OF_WEEK));
	        st.execute();
    	}
		...
		try
		{
			...
			for (StatsSet sevenDat : _signsPlayerData.values())
		    {
		    	...
		    	st = con.prepareStatement(
		    		...
		    		"WHERE char_obj_id=?");
		    	st.setString(1, sevenDat.getString("cabal"));
		    	st.setInt(2, sevenDat.getInteger("seal"));
		    	st.setInt(3, sevenDat.getInteger("red_stones"));
		    	st.setInt(4, sevenDat.getInteger("green_stones"));
		    	st.setInt(5, sevenDat.getInteger("blue_stones"));
		    	st.setDouble(6, sevenDat.getDouble("ancient_adena_amount"));
		    	st.setDouble(7, sevenDat.getDouble("contribution_score"));
		    	st.setInt(8, sevenDat.getInteger("char_obj_id"));
		    	st.execute();
		    	Close.S(st);
		    }
	        ...
	        if (updateSettings)
	        {
	        	TextBuilder sqlQuery = new TextBuilder("UPDATE seven_signs_status SET current_cycle=?, active_period=?, previous_winner=?, " +
	            ...
	            for (int i = 0; i < (SevenSignsFestival.FESTIVAL_COUNT); i++)
	                sqlQuery.append("accumulated_bonus" + String.valueOf(i) + "=?, ");

                sqlQuery.append("date=? WHERE id=0");

	            st = con.prepareStatement(sqlQuery.toString());
	            st.setInt(1, _currentCycle);
	            st.setInt(2, _activePeriod);
	            st.setInt(3, _previousWinner);
	            st.setDouble(4, _dawnStoneScore);
	            st.setInt(5, _dawnFestivalScore);
	            st.setDouble(6, _duskStoneScore);
	            st.setInt(7, _duskFestivalScore);
	            st.setInt(8, _signsSealOwners.get(SEAL_AVARICE));
	            st.setInt(9, _signsSealOwners.get(SEAL_GNOSIS));
	            st.setInt(10, _signsSealOwners.get(SEAL_STRIFE));
	            st.setInt(11, _signsDawnSealTotals.get(SEAL_AVARICE));
	            st.setInt(12, _signsDawnSealTotals.get(SEAL_GNOSIS));
	            st.setInt(13, _signsDawnSealTotals.get(SEAL_STRIFE));
	            st.setInt(14, _signsDuskSealTotals.get(SEAL_AVARICE));
	            st.setInt(15, _signsDuskSealTotals.get(SEAL_GNOSIS));
	            st.setInt(16, _signsDuskSealTotals.get(SEAL_STRIFE));
	            ...
	            st.setInt(18 + SevenSignsFestival.FESTIVAL_COUNT, Calendar.getInstance().get(Calendar.DAY_OF_WEEK));
	            st.execute();
	        }
		}
		catch (SQLException e)
	...
	public int setPlayerInfo(L2PcInstance player, int chosenCabal, int chosenSeal)
	{
		...
		else
		{
			...
			{
				con = L2DatabaseFactory.get();
				st = con.prepareStatement("INSERT INTO seven_signs (char_obj_id, cabal, seal) VALUES (?,?,?)");
				st.setInt(1, charObjId);
				st.setString(2, getCabalShortName(chosenCabal));
				st.setInt(3, chosenSeal);
				st.execute();
			}
			catch (SQLException e)
			{
	...
	public void sendCurrentPeriodMsg(L2PcInstance player)
	{
        ...
        {
            case PERIOD_COMP_RECRUITING:
                sm = Static.PREPARATIONS_PERIOD_BEGUN;
                break;
            case PERIOD_COMPETITION:
                sm = Static.COMPETITION_PERIOD_BEGUN;
                break;
            case PERIOD_COMP_RESULTS:
                sm = Static.RESULTS_PERIOD_BEGUN;
                break;
            case PERIOD_SEAL_VALIDATION:
                sm = Static.VALIDATION_PERIOD_BEGUN;
                break;
        }
	...
	{
        SystemMessage sm = SystemMessage.id(sysMsgId);
	...
	public void sendMessageToAll(SystemMessage msg)
	{
		for (L2PcInstance player : L2World.getInstance().getAllPlayers())
		{
			if (player == null)
			...
			switch (currSeal)
			{
				case SEAL_AVARICE:
					if (newSealOwner == CABAL_DAWN)
						sendMessageToAll(Static.DAWN_OBTAINED_AVARICE);
					else if (newSealOwner == CABAL_DUSK)
						sendMessageToAll(Static.DUSK_OBTAINED_AVARICE);
					break;
				case SEAL_GNOSIS:
					if (newSealOwner == CABAL_DAWN)
						sendMessageToAll(Static.DAWN_OBTAINED_GNOSIS);
					else if (newSealOwner == CABAL_DUSK)
						sendMessageToAll(Static.DUSK_OBTAINED_GNOSIS);
					break;
				case SEAL_STRIFE:
					if (newSealOwner == CABAL_DAWN)
						sendMessageToAll(Static.DAWN_OBTAINED_STRIFE);
					else if (newSealOwner == CABAL_DUSK)
						sendMessageToAll(Static.DUSK_OBTAINED_STRIFE);
        ...
        for (L2PcInstance onlinePlayer : L2World.getInstance().getAllPlayers())
        {
            ...
            if (isSealValidationPeriod() || isCompResultsPeriod())
            {
                if (!onlinePlayer.isGM() && onlinePlayer.isIn7sDungeon() && !currPlayer.getString("cabal").equals(compWinner))
                {
                    ...
                    onlinePlayer.sendPacket(Static.SEAL_VAL1);
                }
            }
            else
            {
                if (!onlinePlayer.isGM() && onlinePlayer.isIn7sDungeon() && !currPlayer.getString("cabal").equals(""))
                {
                    onlinePlayer.teleToLocation(MapRegionTable.TeleportWhereType.Town);
                    onlinePlayer.setIsIn7sDungeon(false);
                    onlinePlayer.sendPacket(Static.SEAL_VAL2);
                }
            }
        }
    }
        ...
        public void run()
        {
	        ...
	        switch (periodEnded)
	        {
	            case PERIOD_COMP_RECRUITING: // Initialization
                    ...
                    sendMessageToAll(Static.QUEST_EVENT_PERIOD_BEGUN);
	                break;
	            case PERIOD_COMPETITION: // Results Calculation

	                // Send message that Competition has ended.
	                sendMessageToAll(Static.QUEST_EVENT_PERIOD_ENDED);
                    ...
                    {
                    	case CABAL_DAWN:
                    		sendMessageToAll(Static.DAWN_WON);
                    		break;
                    	case CABAL_DUSK:
                    		sendMessageToAll(Static.DUSK_WON);
                    		break;
                    }
	            ...
	            case PERIOD_COMP_RESULTS: // Seal Validation
                    ...
                    sendMessageToAll(Static.SEAL_VALIDATION_PERIOD_BEGUN);
	            ...
	            case PERIOD_SEAL_VALIDATION: // Reset for New Cycle
	                ...
	                sendMessageToAll(Static.SEAL_VALIDATION_PERIOD_ENDED);
