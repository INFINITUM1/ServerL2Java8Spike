...
{
	protected static final Logger _log = AbstractLogger.getLogger(PetitionManager.class.getName());
		...
		public boolean endPetitionConsultation(PetitionState endState)
		{
			...
			if (getResponder() != null && getResponder().isOnline() == 1)
			{
				...
				else
				{
					...
					getResponder().sendPacket(SystemMessage.id(SystemMessageId.PETITION_ENDED_WITH_S1).addString(getPetitioner().getName()));

					if (endState == PetitionState.Petitioner_Cancel)
					{
                        // Receipt No. <ID> petition cancelled.
						getResponder().sendPacket(SystemMessage.id(SystemMessageId.RECENT_NO_S1_CANCELED).addNumber(getId()));
					}
				}
			}
			...
			if (getPetitioner() != null && getPetitioner().isOnline() == 1)
				getPetitioner().sendPacket(Static.THIS_END_THE_PETITION_PLEASE_PROVIDE_FEEDBACK);
	...
	private PetitionManager()
	{
		...
		_log.info("PetitionManager: Loaded.");
	}
	...
	public boolean acceptPetition(L2PcInstance respondingAdmin, int petitionId)
	{
		...
		currPetition.sendPetitionerPacket(Static.PETITION_APP_ACCEPTED);

        // Petition application accepted. Reciept No. is <ID>
		currPetition.sendResponderPacket(SystemMessage.id(SystemMessageId.PETITION_ACCEPTED_RECENT_NO_S1).addNumber(currPetition.getId()));

        // Petition consultation with <Player> underway.
		currPetition.sendResponderPacket(SystemMessage.id(SystemMessageId.PETITION_WITH_S1_UNDER_WAY).addString(currPetition.getPetitioner().getName()));
		return true;
	}
