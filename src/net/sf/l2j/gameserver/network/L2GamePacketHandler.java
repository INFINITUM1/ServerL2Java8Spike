...
import org.mmocore.network.nio.impl.IClientFactory;
import org.mmocore.network.nio.impl.IMMOExecutor;
import org.mmocore.network.nio.impl.IPacketHandler;
import org.mmocore.network.nio.impl.MMOConnection;
import org.mmocore.network.nio.impl.ReceivablePacket;

/**
 * Stateful Packet Handler<BR> The Stateful approach prevents the server from
 * handling inconsistent packets, examples:<BR> <li>Clients sends a
 * MoveToLocation packet without having a character attached. (Potential errors
 * handling the packet).</li> <li>Clients sends a RequestAuthLogin being already
 * authed. (Potential exploit).</li> <BR><BR> Note: If for a given exception a
 * packet needs to be handled on more then one state, then it should be added to
 * all these states.
 *
 * @author KenM
 */
public final class L2GamePacketHandler implements IPacketHandler<L2GameClient>, IClientFactory<L2GameClient>, IMMOExecutor<L2GameClient> {
    ...
    public ReceivablePacket<L2GameClient> handlePacket(ByteBuffer buf, L2GameClient client) {
        ...
        //return null;
        if (client == null || client.isFlooder()) {
            return null;
        }
        if (!buf.hasRemaining()) {
            return null;
...
//						break;
    ...
    @Override
    public void execute(Runnable rp) {
