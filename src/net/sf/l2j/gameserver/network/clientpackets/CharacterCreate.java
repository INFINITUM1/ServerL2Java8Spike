...
public final class CharacterCreate extends L2GameClientPacket {
    ...
    // Logger.getLogger(CharacterCreate.class.getName());
    // cSdddddddddddd
    ...
    protected void runImpl() {
        if (CharNameTable.getInstance()
                .accountCharNumber(getClient().getAccountName()) >= Config.MAX_CHARACTERS_NUMBER_PER_ACCOUNT
            ...
            return;
        } else if ((_name.length() < 3) || (_name.length() > 16) || !Util.isAlphaNumeric(_name)
                || !isValidName(_name)) {
        ...
        L2PcInstance newChar = L2PcInstance.create(objectId, template, getClient().getAccountName(), _name, _hairStyle,
    ...
    private static final Pattern cnamePattern = Pattern.compile(Config.CNAME_TEMPLATE);// Pattern.compile("[\\w\\u005F\\u002E]+",
    ...
    private void initNewChar(L2GameClient client, L2PcInstance newChar) {
        ...
        if (Config.CUSTOM_SHORTCUTS) {
            ...
            for (int i = 0; i < startSkills.length; i++) {
                ...
                // if (Config.DEBUG)
                // _log.fine("adding starter skill:" + startSkills[i].getId()+ " / "+
                // startSkills[i].getLevel());
            }
        }
        // L2ShortCut shortcut;

        // if (ALT_START_ITEMS) {
        // L2ItemInstance item4 = newChar.getInventory().addItem("Init", 728, 1,
        // newChar, null);
        // shortcut = new L2ShortCut(5, 0, 1, item4.getObjectId(), -1, 1);
        // newChar.registerShortCut(shortcut);

        // L2ItemInstance item2 = newChar.getInventory().addItem("Init", 1467, 1,
        // newChar, null);
        // shortcut = new L2ShortCut(8, 0, 1, item2.getObjectId(), -1, 1);
        // newChar.registerShortCut(shortcut);

        // L2ItemInstance item3 = newChar.getInventory().addItem("Init", 3952, 1,
        // newChar, null);
        ...
        // for (Integer itemId : template.getItems()) {
        // L2ItemInstance item = newChar.getInventory().addItem("Init", itemId, 1,
        // newChar, null);
        // if (item.getItemId() == 5588) {
        // // add tutbook shortcut
        // shortcut = new L2ShortCut(11, 0, 1, item.getObjectId(), -1, 1);
        ...
        // if (Config.CHAR_CREATE_ENCHANT > 0) {
        // item.setEnchantLevel(Config.CHAR_CREATE_ENCHANT);
        // // if (newChar.getActiveWeaponItem() != null && item.getItem().getType2() ==
        // // L2Item.TYPE2_WEAPON)
        // // continue;
        // }
        // newChar.getInventory().equipItemAndRecord(item);
        ...
        // for (FastMap.Entry<Integer, Integer> e = buffs.head(), end = buffs.tail(); (e
        // = e.getNext()) != end;) {
        ...
        // L2SkillLearn[] startSkills =
        // SkillTreeTable.getInstance().getAvailableSkills(newChar,
        // newChar.getClassId());
        // for (int i = 0; i < startSkills.length; i++) {
        // newChar.addSkill(SkillTable.getInstance().getInfo(startSkills[i].getId(),
        // startSkills[i].getLevel()),
        // true);
        // if (startSkills[i].getId() == 1001 || startSkills[i].getId() == 1177) {
        // shortcut = new L2ShortCut(1, 0, 2, startSkills[i].getId(), 1, 1);
