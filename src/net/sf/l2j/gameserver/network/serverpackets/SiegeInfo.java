...
public class SiegeInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
    {
		...
		if (_castle.getCastleId() != 34 && _castle.getCastleId() != 64)
		{
		...
		else if (_castle.getCastleId() == 34 || _castle.getCastleId() == 64)
		{
			int CHowner = ClanHallManager.getInstance().getClanHallById(_castle.getCastleId()).getOwnerId();
			writeD(((CHowner == activeChar.getClanId()) && (activeChar.isClanLeader())) ? 0x01 : 0x00);
			writeD(CHowner);
			if (CHowner > 0)
			{
				L2Clan owner = ClanTable.getInstance().getClan(CHowner);
				if (owner != null)
				{
					writeS(owner.getName());        // Clan Name
					writeS(owner.getLeaderName());  // Clan Leader Name
