...
public class SelectorThread<T extends MMOClient> extends Thread {
    ...
    //private static final Logger _log = Logger.getLogger(SelectorThread.class);
    ...
    protected void readPacket(SelectionKey key) {
        ...
        if (buf.position() == buf.limit()) {
            ...
            System.out.println("Read buffer exhausted for client : " + con.getClient() + ", try to adjust buffer size, current : " + buf.capacity() + ", primary : " + (buf == READ_BUFFER) + ". Closing connection.");
            closeConnectionImpl(con);
        } else {
    ...
    protected boolean tryReadPacket2(SelectionKey key, MMOConnection<T> con, ByteBuffer buf) {
        ...
        if (buf.remaining() > _sc.HEADER_SIZE) {
            ...
            if (size <= _sc.HEADER_SIZE || size > _sc.PACKET_SIZE) {
                ...
                System.out.println("Incorrect packet size : " + size + "! Client : " + con.getClient() + ". Closing connection.");
                closeConnectionImpl(con);
                return false;
            }
        ...
        if (pos == buf.capacity()) {
            ...
            System.out.println("Read buffer exhausted for client : " + con.getClient() + ", try to adjust buffer size, current : " + buf.capacity() + ", primary : " + (buf == READ_BUFFER) + ".");
        }
